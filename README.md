...
- **No bot support.** Because the selected map is loaded as a spawn group into the running lobby level rather than a real level transition, bots are sadly not supported.
- **The minimap is a workaround.** For the same reason the loaded map's own radar never activates. Instead, the lobby level contains one disabled `cs_minimap_volume` per map (and per lower level on nuke/train/vertigo), and the script enables the loaded map's volume(s), re-enabling them every round since round restarts reset them. These volumes, their radar images and overview entries are all generated by `npm run update-minimap`.
...
- `npm run update-minimap` — regenerate the minimap setup from `src/minimap-generator/minimap.config.json`: the `maps/prefabs/dynamic_minimap.vmap` prefab (one `cs_minimap_volume` per entry, sized to the radar's official `pos_x`/`pos_y`/`scale`), each radar's TGA (from `src/minimap-generator/images/`), texture settings and `.vpdi`, and the `Volumes` block of the game-side `resource/overviews/but_damage_throws_nades.txt`. Don't use Hammer's "generate minimap" afterwards - it overwrites these.
